...
第一章：触发与流程（你在这里）  ← 了解入口
第二章：输入理解              ← 接受热点素材
第三章：研究与发散            ← 具体执行步骤（步骤1-4）
第四章：三件套产出            ← skill + 日志 + 文章规范
第五章：GitHub 发布流程       ← 含 Fallback 方案
附录：模板文件              ← references/ 下的模板和案例
```
...
### Phase 1.5：研究质量 Review 检查点

所有竞品分析完成后，暂停展示研究质量摘要：
...
**检查点意义**：研究质量决定切入点判断的质量。垃圾输入垃圾输出，在这里拦截比返工成本低得多。

用户确认研究质量 OK → 进入 Phase 2。
用户觉得某维度不够 → 补充调研后再继续。
...
**适用条件**：当输入是「人物/角色/思维框架」类素材时（如 GitHub 上的 persona skill、某个人的作品集）。

如果不是人物类素材（如工具类产品、数据分析框架），跳过此 Phase，直接进入 Phase 3。

#### Phase 2.1：创建研究目录

```bash
mkdir -p {skill-name}/references/research/
```

#### Phase 2.2：六维并行研究

Spawn 6 个并行 subagent（如果输入是人物类素材）或 3 个（如果是主题类素材）。

详细维度定义见 `references/research-framework.md`。
...
读取 `references/extraction-framework.md`，对 6 个维度的素材执行结构化提炼。

**核心输出：**
- **心智模型 3-7 个**（三重验证全通过）：每个含名称/证据/应用/局限性
- **决策启发式 5-10 条**（仅 1 重通过）：「如果X，则Y」+ 案例
- **内在张力 1-2 对**：此人价值观之间的真实矛盾
- **诚实边界**：这个 skill 的局限性

**提炼检查（自动执行）：**
- [ ] 数量 3-7 个心智模型（<3=信息不足，>7=提炼不够）
- [ ] 每条有局限性（只写优点=不合格）
- [ ] 有内在矛盾（高度一致是假的）
- [ ] 每条有来源证据（无来源=推测，需要标注）
...
基于研究，回答三个问题（这是 Phase 2 的输出，也是第三阶段的输入）：

1. **我能解决什么问题？**（现有方案做不到或做不好的）
2. **我用什么方式解决？**（不是复刻，是差异化的方法）
3. **这个 skill 的独特价值主张是什么？**（一句话说清楚）

---

### Phase 2.5：提炼确认检查点

Phase 2 完成后，暂停展示提炼摘要给用户确认：
...
**检查点意义**：提炼是主观判断最重的环节，确认后再构建，避免写完 400 行 SKILL.md 才发现方向不对。

用户确认 OK → 进入第三阶段。
用户觉得某个模型不对或缺少 → 回到 Phase 2.3 调整后再继续。
